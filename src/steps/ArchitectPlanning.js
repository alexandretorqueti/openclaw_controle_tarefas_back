...
exports.passoArchitectPlanning = void 0;
...
class ArchitectPlanningStep {
     ...
     */
    constructor(options = {}) {
        this.log = options.log || container_1.default.resolve('log');
        // Usar instâncias fornecidas ou criar do container
        this.openClawService = options.openClawService || container_1.default.resolve('openClawService');
        this.sessionChainUtils = options.sessionChainUtils || container_1.default.resolve('sessionChainUtils');
        this.smartFileFinder = options.smartFileFinder || container_1.default.resolve('smartFileFinder');
        this.taskAnalysisService = options.taskAnalysisService || container_1.default.resolve('taskAnalysisService');
        this.workspaceSnapshotService = options.workspaceSnapshotService || container_1.default.resolve('workspaceSnapshotService');
        this.fileUtils = options.fileUtils || container_1.default.resolve('fileUtils');
        this.fileSystem = options.fileSystem || container_1.default.resolve('fileSystem');
        this.promptFactory = options.promptFactory || container_1.default.resolve('promptFactory');
    }
    /**
    ...
    async execute(context) {
        const { tarefaAtual, project, files, initialSnapshot, config, analysisPlan, developerPrompt = '', currentInput = '' } = context;
        ...
        try {
            ...
            const architectResult = await this.openClawService.executeWithFallback(architectSessionId, architectInput, project?.agent || tarefaAtual.agent || 'main', tarefaAtual.agent || 'main', null, config.TASKS_DIR, files.architectLogFile, project?.pastaBase, config.TASK_TIMEOUT_MS);
            let architectPlan = "";
            let architectAnalysis = null;
            // =====================================================================
            // PASSO 1: Busca inteligente pelo plano do arquiteto
            // =====================================================================
            const planSearch = await this.smartFileFinder.findRealArchitectPlan(files.architectPlanFile, config.TASKS_DIR, 5);
            ...
            if (architectPlan && architectPlan.trim().length > 0) {
                ...
                architectAnalysis = await this.taskAnalysisService.analyzeArchitectResponse(architectPlan, tarefaAtual, project, { hasRealChanges, existsDoneFile, changes } // <-- Passando as provas!
                );
...
exports.passoArchitectPlanning = {
    name: 'Planejamento do Arquiteto',
    func: async (ctx) => {
        const architectPlanningStep = new ArchitectPlanningStep();
        const result = await architectPlanningStep.execute(ctx);
    }
};
// ============================================================================
// EXPORTAÇÕES
// ============================================================================
exports.default = ArchitectPlanningStep;
