...
class TaskExecutionService {
    static readTaskOutputFile(taskId, baseDir, fileType) {
    ...
    // ==========================================
    static createExecutionLog(task, userId, agentId) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            let model = 'deepseek/deepseek-chat';
            if (agentId) {
                try {
                    const agentDetails = yield agentService.getAgentDetails(agentId);
                    model = ((_a = agentDetails.identity) === null || _a === void 0 ? void 0 : _a.model) || model;
                }
                catch (error) { /* Silencioso */ }
            }
            // Mantemos o uso do prisma aqui pois TaskExecutionLog é o domínio nativo deste arquivo
            return yield prisma.taskExecutionLog.create({
                data: { taskId: task.id, userId, model, startedAt: new Date(), success: false }
            });
        });
    }
    static finishExecutionLog(logId, result) {
        return __awaiter(this, void 0, void 0, function* () {
            const logEntry = yield prisma.taskExecutionLog.findUnique({ where: { id: logId } });
            if (!logEntry)
                return;
            return yield prisma.taskExecutionLog.update({
                where: { id: logId },
                data: {
                    finishedAt: new Date(),
    ...
    /**
       * Passo 1: Prepara arquivos, banco de dados e analisa o escopo.
       */
    static stepSetupContext(ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            const { task, userId, config } = ctx;
            const { TASKS_DIR } = config;
            const project = task.projectId ? yield prisma.project.findUnique({ where: { id: task.projectId } }) : null;
            const taskId = task.id;
            // 1. Analisa o escopo da tarefa para definir o taskType (development, analysis, automation)
            const analysisPlan = yield TaskAnalysisService.analyzeTaskScope(task, project, path.join(TASKS_DIR, `terminal-pre-analise-${taskId}.log`));
            ...
            const dirBase = (project === null || project === void 0 ? void 0 : project.pastaBase) || 'Diretório atual';
            ...
            const initialSnapshot = yield WorkspaceSnapshotService.takeSnapshot((project === null || project === void 0 ? void 0 : project.pastaBase) || TASKS_DIR);
            ...
            const architectPrompt = PromptFactory.buildArchitectPrompt(task, project, fileList, files.architectPlanFile, commentsSection, analysisPlan.taskType // <-- Injetando o tipo exato para o prompt condicional!
            );
            // 6. Salva nos arquivos físicos
            const architectPromptBackupFile = path.join(TASKS_DIR, `architect-prompt-${taskId}.txt`);
            yield fs.writeFile(architectPromptBackupFile, architectPrompt);
            yield fs.writeFile(files.promptFile, architectPrompt);
            yield fs.writeFile(files.terminalLogFile, '');
            ...
            const executionLog = yield TaskExecutionService.createExecutionLog(task, userId, task.agent);
            return Object.assign(Object.assign({}, ctx), { project,
                analysisPlan,
                files,
                initialSnapshot,
                executionLog, currentInput: architectPrompt, developerPrompt,
                commentsSection });
        });
    }
    /**
     ...
     */
    static stepArchitectPlanning(ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            const { task, project, files, initialSnapshot, config, analysisPlan } = ctx;
            // Log do tipo de tarefa
            yield log(`📋 [Arquiteto] Tipo de tarefa: ${analysisPlan.taskType}`);
            const fileList = Array.from(initialSnapshot.keys());
            ...
            yield log(`🧠 [Arquiteto] Avaliando a tarefa ${task.id} e montando o plano de ação...`);
            ...
            const architectSessionId = yield SessionChainUtils.generateUnifiedSessionId(task.id, 'arquiteto');
            yield log(`🔗 Sessão do arquiteto: ${architectSessionId} (baseada na cadeia de dependências)`);
            // Roda o Arquiteto com timeout de 10 minutos (600000ms)
            const architectResult = yield OpenClawService.executeWithFallback(architectSessionId, // <--- SESSÃO UNIFICADA PARA CADEIAS DE DEPENDÊNCIAS
            architectInput, (project === null || project === void 0 ? void 0 : project.agent) || task.agent || 'main', task.agent || 'main', null, config.TASKS_DIR, files.architectLogFile, project === null || project === void 0 ? void 0 : project.pastaBase, config.TASK_TIMEOUT_MS);
            let architectPlan = "";
            let architectAnalysis = null;
            // === A NOVA INTELIGÊNCIA DE BUSCA ENTRA AQUI ===
            const planSearch = yield SmartFileFinder.findRealArchitectPlan(files.architectPlanFile, config.TASKS_DIR, 5);
            if (planSearch.content) {
                architectPlan = planSearch.content;
                yield log(`📝 [Arquiteto] Plano recuperado com sucesso (${architectPlan.length} caracteres).`);
            }
                ...
                yield log(`📝 [Arquiteto] Arquivo não encontrado. Usando rawOutput do terminal como fallback (${architectPlan.length} chars)`);
                // Força a gravação no arquivo correto
                yield fs.writeFile(files.architectPlanFile, architectPlan).catch(() => { });
            }
            ...
            if (architectPlan && architectPlan.trim().length > 0) {
                ...
                yield log(`🧠 [Arquiteto] Analisando resposta com IA...`);
                architectAnalysis = {};
                const existsDoneFile = yield fileExists(files.doneFile);
                architectAnalysis = yield TaskAnalysisService.analyzeArchitectResponse(architectPlan, task, project);
                yield log(`📊 [Arquiteto] Análise inicial: hasExecuted=${architectAnalysis.hasExecuted}, hasPlan=${architectAnalysis.hasPlan}, confidence=${architectAnalysis.confidence}%`);
                ...
                if ((architectAnalysis.hasExecuted && architectAnalysis.confidence > 70) || existsDoneFile) {
                    yield log(`🔍 [Validação] IA diz que arquiteto executou. Verificando evidências...`);
                    // Verificar se há evidências reais de execução
                    const currentSnapshot = yield WorkspaceSnapshotService.takeSnapshot((project === null || project === void 0 ? void 0 : project.pastaBase) || ctx.config.TASKS_DIR);
                    ...
                    const architectDoneExists = yield fileExists(files.doneFile);
                    const architectReportExists = yield fileExists(files.relatorioFile);
                    ...
                    if (!hasEvidence) {
                        yield log(`⚠️ [Validação] NENHUMA evidência encontrada! IA provavelmente errou. Corrigindo análise...`);
                        ...
                        yield log(`📊 [Arquiteto] Análise CORRIGIDA: hasExecuted=false, hasPlan=true (falta de evidências)`);
                    }
                    else {
                        if (analysisPlan.taskType === 'analysis') {
                            yield log(`✅ [Validação] Evidências confirmadas para análise: relatório/.done criados`);
                        }
                        else {
                            yield log(`✅ [Validação] Evidências confirmadas: ${changes.modified.length} arquivos modificados, ${changes.created.length} criados`);
                        }
                    }
                }
                if (architectAnalysis.hasExecuted) {
                    yield log(`✅ [Arquiteto] Análise indica que já executou a tarefa (${architectAnalysis.confidence}% confiança).`);
                    if (architectAnalysis.executionDetails) {
                        yield log(`📝 [Arquiteto] Detalhes: ${architectAnalysis.executionDetails.substring(0, 100)}...`);
                    }
                }
                else if (architectAnalysis.hasPlan) {
                    yield log(`📋 [Arquiteto] Análise indica que gerou plano de ação (${architectAnalysis.confidence}% confiança).`);
                    if (architectAnalysis.planDetails) {
                        yield log(`📝 [Arquiteto] Detalhes: ${architectAnalysis.planDetails.substring(0, 100)}...`);
                    }
                }
                else if (architectAnalysis.analysisFailed) {
                    yield log(`⚠️ [Arquiteto] Análise falhou ou resposta incompreensível.`);
                }
                else {
                    yield log(`ℹ️ [Arquiteto] Análise não identificou execução nem plano claro.`);
                }
            }
            else {
                ...
                yield log(`⚠️ [Arquiteto] Resposta vazia ou inválida. Usando descrição original.`);
            }
            ...
            if (architectAnalysis.hasExecuted && architectAnalysis.confidence > 70 && architectPlan.trim()) {
                ...
                yield log(`🔄 [Arquiteto] Fluxo: Usando execução do arquiteto (alta confiança + evidências validadas).`);
            }
                ...
                yield log(`🔄 [Arquiteto] Fluxo: Substituindo prompt pelo plano + instruções do desenvolvedor.`);
            }
                ...
                yield log(`🔄 [Arquiteto] Fluxo: Mantendo prompt original (análise falhou).`);
            }
            else {
                // Caso padrão (baixa confiança, resposta ambígua)
                updatedPromptContent = `${ctx.currentInput}\n\n=== ANÁLISE DO ARQUITETO ===\n${architectPlan}\n\nAnalise a resposta acima e execute conforme necessário.`;
                yield log(`🔄 [Arquiteto] Fluxo: Resposta ambígua, incluindo análise como referência.`);
            }
            yield fs.writeFile(files.promptFile, updatedPromptContent);
            return Object.assign(Object.assign({}, ctx), { currentInput: updatedPromptContent, architectAnalysis,
                architectPlan });
        });
    }
    /**
     ...
     */
    static verifyArchitectWork(ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield log(`🔍 [Verificação] Arquiteto indica execução (${architectAnalysis.confidence}% confiança).`);
            ...
            if (!architectLeftEvidence) {
                yield log(`⚠️ [Verificação] Arquiteto não deixou evidências de execução.`);
            ...
            if (analysisPlan.taskType !== 'analysis' && !hasRealChanges) {
                yield log(`📝 [Verificação] Arquiteto deixou .done/relatório, mas não alterou arquivos (e a tarefa não é de análise).`);
            ...
            if (analysisPlan.taskType === 'analysis') {
                yield log(`📝 [Verificação] Tarefa de análise: ${changes.modified.length} arquivos modificados, ${changes.created.length} criados (alterações são opcionais para análise).`);
            }
            else {
                yield log(`📁 [Verificação] Arquiteto alterou ${changes.modified.length} arquivos, criou ${changes.created.length}.`);
            }
            // Verificar contrato
            const contractResult = yield ContractVerificationService.verifyContract(files.doneFile, files.relatorioFile, files.terminalLogFile, {
            ...
            if (!contractResult.contractFulfilled) {
                yield log(`⚠️ [Verificação] Contrato não cumprido pelo arquiteto: ${contractResult.executionNotes}`);
            ...
            yield log(`✅ [Verificação] Contrato cumprido pelo arquiteto.`);
            // Para tarefas de desenvolvimento, verificar ecossistema (build, testes)
            if (analysisPlan.taskType === 'development') {
                yield log(`🔧 [Verificação] Verificando ecossistema (build, testes)...`);
                const qa = yield TaskExecutionService.ensureAndValidateEcosystem(task, project, ctx.config);
                if (!qa.passed) {
                    yield log(`❌ QA Reprovado. Build falhou: ${qa.message}`);
                    yield fs.unlink(files.doneFile).catch(() => { });
                ...
                yield log(`✅ [Verificação] Ecossistema verificado com sucesso!`);
                return {
                    success: true,
                    contractResult: {
                ...
                yield log(`✅ [Verificação] Tarefa de ${analysisPlan.taskType} concluída pelo arquiteto.`);
                return {
                    success: true,
                    contractResult: {
                        contractFulfilled: true,
     ...
     */
    static ensureAndValidateEcosystem(task, project, config) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const tryExecutingCommand = (cmd, appName, appPath, port) => __awaiter(this, void 0, void 0, function* () {
                var _a, _b;
                try {
                    yield log(`⏳ [Sistema 1] Validando [${appName}] na porta ${port}: ${cmd}`);
                    // --- ATUALIZAÇÃO CIRÚRGICA DO .ENV (Preservando outras variáveis) ---
                    const envPath = path.join(appPath, '.env');
                    let envContent = '';
                    if (yield fileExists(envPath)) {
                        envContent = yield fs.readFile(envPath, 'utf8');
                    }
                    ...
                    yield fs.writeFile(envPath, envContent.trim() + '\n');
                    // ------------------------------------------------------------------
                    execSync(cmd, {
                        cwd: appPath,
                        stdio: 'pipe',
                        timeout: 90000,
                        shell: true,
                        env: Object.assign(Object.assign({}, process.env), { PORT: String(port), PATH: `${nodeBinDir}:${process.env.PATH || ''}` })
                    });
                    return { passed: true };
                }
                catch (e) {
                    const errorLog = (((_a = e.stderr) === null || _a === void 0 ? void 0 : _a.toString()) || ((_b = e.stdout) === null || _b === void 0 ? void 0 : _b.toString()) || e.message);
                    if (e.signal === 'SIGTERM' || e.code === 'ETIMEDOUT' || errorLog.includes('ETIMEDOUT')) {
                        yield log(`✅ [Sistema 1] [${appName}] operacional (Server vivo).`);
                        return { passed: true };
                    }
            ...
            if (domain === 'FRONTEND' && project.frontendBuildCmd && project.frontendPath && project.frontendPort) {
                ...
                return yield tryExecutingCommand(project.frontendBuildCmd, 'FRONTEND', pasta, project.frontendPort || 7000);
            }
            if (domain === 'BACKEND' && project.backendBuildCmd && project.backendPath && project.backendPort) {
                ...
                return yield tryExecutingCommand(project.backendBuildCmd, 'BACKEND', pasta, project.backendPort || 7001);
            }
            ...
            const items = yield fs.readdir(baseDir, { withFileTypes: true });
            const apps = [];
            for (const item of items) {
                if (item.isDirectory() && !item.name.startsWith('.') && item.name !== 'node_modules') {
                    const appDir = path.join(baseDir, item.name);
                    if (yield fileExists(path.join(appDir, 'package.json'))) {
                        apps.push({ name: item.name, path: appDir });
                    }
                }
            }
            for (const app of apps) {
                yield log(`🔍 [Sistema 1] Analisando módulo: [${app.name}]`);
                ...
                if (!cmd) {
                    yield log(`🧠 [Sistema 1] Novo módulo detectado [${app.name}]. IA definindo escopo...`);
                    const pkgContent = yield fs.readFile(path.join(app.path, 'package.json'), 'utf8');
                    const ai = new LlmService();
                    const decision = yield ai.analyze(`Analise este package.json e retorne JSON: {"comando": "string", "escopo": "front|back"}\n\n${pkgContent}`);
                    cmd = (decision === null || decision === void 0 ? void 0 : decision.comando) || 'npm start';
                    const escopo = (decision === null || decision === void 0 ? void 0 : decision.escopo) || (app.name.toLowerCase().includes('front') ? 'front' : 'back');
                    ...
                    yield projectService.updateProject(project.id, project);
                }
                ...
                if (!(yield fileExists(envPath))) {
                    yield fs.writeFile(envPath, `PORT=${finalPort}`);
                }
                // 5. EXECUÇÃO (O "Fast Path" e o "Discovery Path" se encontram aqui)
                const result = yield tryExecutingCommand(cmd, app.name, app.path, finalPort);
                if (!result.passed)
                    return result;
            }
            yield log(`✅ [Sistema 1] Todos os módulos operacionais.`);
            return { passed: true };
        });
    }
    /**
    ...
    */
    static stepDeveloperLoop(ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const architectVerification = yield TaskExecutionService.verifyArchitectWork(ctx);
            if (architectVerification) {
                if (architectVerification.success) {
                    return Object.assign(Object.assign({}, ctx), { contractResult: architectVerification.contractResult, finalResult: architectVerification.finalResult });
                }
                else if (architectVerification.needsDeveloper) {
                    currentInput = architectVerification.message;
                    yield fs.writeFile(files.promptFile, currentInput);
                    yield log(`🔄 [Desenvolvedor] Necessita intervenção: ${architectVerification.message.substring(0, 100)}...`);
                }
            }
            else {
            ...
            try {
                const taskChain = yield SessionChainUtils.getTaskChain(task.id);
                ...
                if (previousTasks.length > 0) {
                    ...
                    for (const prevTask of previousTasks) {
                        const prevReport = yield TaskExecutionService.readTaskOutputFile(prevTask.id, config.TASKS_DIR, 'relatorio');
                        const prevTerminal = yield TaskExecutionService.readTaskOutputFile(prevTask.id, config.TASKS_DIR, 'terminal');
                    ...
                    yield log(`📚 [Desenvolvedor] Contexto de ${previousTasks.length} tarefas anteriores carregado.`);
                }
            }
            catch (chainErr) {
                yield log(`⚠️ [Desenvolvedor] Erro ao buscar cadeia de dependências: ${chainErr.message}`);
            }
            ...
            while (!contractResult.contractFulfilled && turnos < 15) {
                ...
                yield fs.writeFile(developerPromptFile, promptDesteTurno).catch(() => { });
                // 3. CHAMA O OPENCLAW
                const res = yield OpenClawService.executeWithFallback(turnSessionId, promptDesteTurno, task.agent || (project === null || project === void 0 ? void 0 : project.agent) || 'main', backupAgent, null, TASKS_DIR, files.terminalLogFile, project === null || project === void 0 ? void 0 : project.pastaBase, TASK_TIMEOUT_MS);
                EvidenceService.applyExecutionEvidence(evidence, res.toolCall, res.toolResult || {}, { executionDirectory: (project === null || project === void 0 ? void 0 : project.pastaBase) || TASKS_DIR });
                contractResult = yield ContractVerificationService.verifyContract(files.doneFile, files.relatorioFile, files.terminalLogFile, {
                    taskType: analysisPlan.taskType,
                    evidence, task, project, analysisPlan, initialSnapshot
                });
                if (contractResult.contractFulfilled) {
                    // Uma única linha aciona a IA de DevOps para varrer e testar tudo!
                    const qa = yield TaskExecutionService.ensureAndValidateEcosystem(task, project, config);
                    if (!qa.passed) {
                        yield fs.unlink(files.doneFile).catch(() => { });
                        lastFeedback = `Validação de Ecossistema falhou:\n\n${qa.message}\n\nCorrija o código no módulo indicado e finalize novamente.`;
                        yield log(`❌ QA Reprovado. Retornando erro para o agente: ${qa.message}`);
                ...
                const findDynamicDone = (dir) => __awaiter(this, void 0, void 0, function* () {
                    if (!dir)
                        return null;
                    try {
                        const dirFiles = yield fs.readdir(dir);
                ...
                const rogueDoneFile = (yield findDynamicDone(config.TASKS_DIR)) || (yield findDynamicDone(project === null || project === void 0 ? void 0 : project.pastaBase));
                if (rogueDoneFile) {
                    doneExists = true;
                ...
                if (doneExists && contractResult.feedbackToAgent) {
                    yield fs.unlink(actualDonePath).catch(() => { });
                    lastFeedback = res.toolFeedback
                        ? `${res.toolFeedback}\n\n[SISTEMA - ATENÇÃO CRÍTICA]\n${contractResult.feedbackToAgent}`
                        : contractResult.feedbackToAgent;
                    yield log(`📝 [Desenvolvedor] Feedback de validação AGENDADO para o próximo turno.`);
                }
                else if (res.toolFeedback) {
                    lastFeedback = res.toolFeedback;
                    yield log(`🛠️ [Desenvolvedor] Resultado da ferramenta AGENDADO para o próximo turno (${lastFeedback.length} chars)`);
                }
                else {
                    ...
                    yield log(`⚠️ [Desenvolvedor] Feedback corretivo AGENDADO.`);
                }
                const prog = EvidenceService.computeTurnProgress(res.toolResult || {}, contractResult, (project === null || project === void 0 ? void 0 : project.pastaBase) || TASKS_DIR);
            ...
            yield log(`📊 [Desenvolvedor] Loop finalizado. contractResult: ${JSON.stringify(contractResult)}`);
            return Object.assign(Object.assign({}, ctx), { contractResult });
        });
    }
    /**
     * Passo 4: Finaliza o log no banco e consolida o resultado.
     */
    static stepTeardown(ctx) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            console.log(`🔧 [DEBUG] stepTeardown chamado para tarefa ${(_a = ctx.task) === null || _a === void 0 ? void 0 : _a.id}`);
            ...
            if (executionLog) {
                yield TaskExecutionService.finishExecutionLog(executionLog.id, finalResult);
            }
            // ==========================================
            ...
            try {
                ...
                const readFileSafe = (filePath) => __awaiter(this, void 0, void 0, function* () {
                    try {
                        if (filePath && (yield fileExists(filePath))) {
                            const content = yield fs.readFile(filePath, 'utf8');
                            yield log(`📄 [Teardown] LIDO: ${path.basename(filePath)} (${content.length} chars)`);
                            return content;
                        }
                    }
                    catch (error) {
                        yield log(`❌ [Teardown] ERRO ao ler ${path.basename(filePath)}: ${error.message}`);
                    }
                    return null;
                });
                ...
                if (!updateData.arquitetosPromptContent) {
                    yield log(`⚠️ [Teardown] Arquivo de prompt do arquiteto não encontrado: ${architectPromptFile}`);
                ...
                if (Object.keys(finalUpdateData).length > 0 && task && task.id) {
                    yield log(`💾 [Teardown] ATUALIZANDO tarefa ${task.id} com ${Object.keys(finalUpdateData).length} campos de log...`);
                    // 3. Substituído a chamada crua do Prisma pelo TaskService!
                    yield taskService.updateTask(task.id, finalUpdateData);
                    yield log(`✅ [Teardown] Tarefa ${task.id} atualizada com sucesso pelo TaskService.`);
                }
                else if (task && task.id) {
                    yield log(`⚠️ [Teardown] Nenhum conteúdo de arquivo encontrado para salvar na tarefa ${task.id}`);
                }
            }
            catch (error) {
                yield log(`💥 [FATAL Teardown] Erro CRÍTICO ao salvar conteúdos no banco: ${error.message}\n${error.stack}`);
            }
            return Object.assign(Object.assign({}, ctx), { finalResult });
        });
    }
    // ==========================================
    // O ORQUESTRADOR PRINCIPAL
    // ==========================================
    static executeTask(task, userId, config) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const finalState = yield runPipeline(`Task-${task.id}`, initialContext, steps);
                return {
                ...
                yield log(`💥 [FATAL] O pipeline falhou: ${error.message}`);
                throw error;
            }
        });
    }
