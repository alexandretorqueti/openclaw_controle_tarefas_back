...
class AgentConfigService {
   ...
   */
  static async getAgentConfig(agentName): Promise<any> {
    try {
   ...
   */
  static async loadAgentContext(agentName): Promise<any> {
    try {
...
class OpenClawService {
  ...
  static async execute(
    ...
    terminalLogFile, 
    projectPath, 
    timeoutMs = 14400000): Promise<any> {
    return new Promise(async (resolve): Promise<any> => {
      ...
      const onData = async (data, source): Promise<any> => {
        if (isSettled) return;
