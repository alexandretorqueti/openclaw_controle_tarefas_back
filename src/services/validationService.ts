...
const { buildValidationPrompt } = promptFactory;
/**
...
class ValidationService {
  constructor() {
    (this as any).cache = new Map(); // Cache simples para validações
    (this as any).defaultModel = 'phi4:latest';
    (this as any).timeoutMs = 5 * 60 * 1000; // 5 minutos
    (this as any).logger = this.createLogger();
  }
   ...
   */
  async validateWithAuxModel(task, project): Promise<any> {
    try {
      // Validações iniciais
      if (!task || !project) {
   ...
   */
  async callWithTimeout(fn, timeoutMs, timeoutMessage): Promise<any> {
    return Promise.race([
      fn(),
   ...
   */
  async validateBatch(tasks, project): Promise<any> {
    const results = [];
