...
class AutoTaskService {
  ...
  async getMonitorProject(): Promise<any> {
  ...
  async findExistingErrorTask(projectId, errorSignature): Promise<any> {
    try {
  ...
  generateErrorDescription(error, req, res) {
    ...
    if (error) {
      ...
      parts.push(`**Status Code:** ${(error as any).statusCode || 'N/A'}`);
    }
  ...
  async getFirstVisibleStatus(): Promise<any> {
    try {
  ...
  async getDefaultPriorityId(): Promise<any> {
    try {
  ...
  async getSystemUserId(): Promise<any> {
    try {
  ...
  async createAutoTask(error, req, res): Promise<any> {
    try {
...
export default new AutoTaskService();