...
// Arquivo: taskHierarchyService.js

export // src/services/taskHierarchyService.js
...
class TaskHierarchyService {
   ...
   */
  async markAncestorsAsChildExecuting(taskId): Promise<any> {
    try {
   ...
   * @returns {Promise<void>}
   */
  async unmarkAncestorsAsChildExecuting(taskId): Promise<any> {
    try {
   ...
   * @returns {Promise<void>}
   */
  async updateAncestorIfNoExecutingChildren(ancestorId): Promise<any> {
    try {
      ...
      if (executingDescendantsCount === 0) {
        ...
        import taskService from "./taskService"; // Require tardio
   ...
   */
  async countExecutingDescendants(taskId): Promise<any> {
    try {
   ...
   */
  async getTaskAncestors(taskId): Promise<any> {
    try {
      const ancestors = [];
   ...
   */
  async getTaskDescendants(taskId): Promise<any> {
    try {
      ...
      const getDescendantsRecursive = async (parentId): Promise<any> => {
   ...
   * @returns {Promise<void>}
   */
  async updateHierarchyOnExecutionChange(taskId, isNowExecuting): Promise<any> {
    try {
...
export default new TaskHierarchyService();