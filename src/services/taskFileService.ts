...
// Arquivo: taskFileService.js

export // src/services/taskFileService.js
// Servico para gerenciamento de arquivos de tarefas

import fs from 'fs/promises';
import * as path from 'path';
import { fileExists, safeMoveFile, safeWriteFile, safeUnlink } from '../utils/fileUtils';
import { formatInlineList, formatNumberedList } from '../utils/formatUtils';
...
class TaskFileService {
   ...
   */
  async moveTaskFiles(taskId, sourceDir, destinationDir): Promise<any> {
    try {
   ...
   */
  async prepareTaskFiles(task, tasksDir, project = null, analysisPlan = null): Promise<any> {
...
${engineRules}`;
   ...
   * @returns {Promise<void>}
   */
  async cleanupFiles(files): Promise<any> {
