...
class CommandExecutor {
     ...
     * @returns {Object}
     */
    static executeTool(toolCall, cwd = process.cwd()) {
        return __awaiter(this, void 0, void 0, function* () {
            const name = toolCall === null || toolCall === void 0 ? void 0 : toolCall.name;
            const args = (toolCall === null || toolCall === void 0 ? void 0 : toolCall.arguments) || {};
            if (!name) {
            ...
            try {
                switch (name) {
                    case 'exec':
                        return yield this.executeExecWithRealMutationCheck(args.command, cwd);
                    case 'read':
    ...
    static buildResult(success, data = {}) {
        ...
        return {
            ...
            stdout: data.stdout || '',
            stderr: data.stderr || '',
            exitCode: (_a = data.exitCode) !== null && _a !== void 0 ? _a : null,
    ...
    static executeExecWithRealMutationCheck(command, cwd = process.cwd()) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log(`   [CommandExecutor] 🖥️  Terminal: ${command.substring(0, 120)}${command.length > 120 ? '...' : ''}`);
            ...
            const before = isMutationAttempt
                ? yield collectFingerprints(candidateFiles)
                : {};
            const shellResult = yield runShellCommand(command, {
                cwd,
                timeout: 240000,
            ...
            const actuallyModifiedFiles = isMutationAttempt
                ...
                : [];
            const noOpMutation = shellResult.success &&
                isMutationAttempt &&
                actuallyModifiedFiles.length === 0;
            const outputParts = [];
            if (String(shellResult.stdout || '').trim())
                outputParts.push(String(shellResult.stdout).trim());
            if (String(shellResult.stderr || '').trim())
                outputParts.push(String(shellResult.stderr).trim());
            if (noOpMutation) {
                outputParts.push('Comando executou com exit code 0, mas não alterou de fato nenhum arquivo-alvo.');
            }
            ...
            if (!success) {
                if (noOpMutation) {
                    ...
                    if (shellResult.errorMessage)
                        errorParts.push(shellResult.errorMessage);
                    if (String(shellResult.stderr || '').trim())
                        errorParts.push(String(shellResult.stderr).trim());
    ...
    static executeWriteCommand(filePath, content, cwd) {
        ...
        try {
            const resolvedPath = this.resolveFilePath(filePath, cwd);
            const normalizedContent = typeof content === 'string' ? content : String(content !== null && content !== void 0 ? content : '');
    ...
    static executeEditCommand(filePath, oldText, newText, cwd) {
        ...
        try {
            ...
            const normalizedNewText = typeof newText === 'string' ? newText : String(newText !== null && newText !== void 0 ? newText : '');
...
function collectFingerprints(files = []) {
    return __awaiter(this, void 0, void 0, function* () {
        const result = {};
        for (const file of uniquePaths(files)) {
            result[file] = yield fingerprintFile(file);
        }
        return result;
    });
}
...
function runShellCommand(command, options = {}) {
    return new Promise((resolve) => {
        ...
        exec(command, {
            ...
            const exitCode = typeof (error === null || error === void 0 ? void 0 : error.code) === 'number'
                ? error.code
                : error
                    ? 1
                    : 0;
            resolve({
                ...
                stdout,
                stderr,
                errorMessage: (error === null || error === void 0 ? void 0 : error.message) || null
            });
        });
    });
}
