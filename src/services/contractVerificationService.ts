...
import { resolveProjectPath, isPathInside } from '../utils/pathUtils';
import { isInspectionCommand, commandTargetsOnlySpecialFiles, isMeaningfulCommand } from '../utils/commandUtils';
import TaskAnalysisService from "./taskAnalysisService";
import WorkspaceSnapshotService from "./workspaceSnapshotService"; // <-- Nova importação
...
/**
  ...
  static async verifyContract(doneFile, relatorioFile, terminalLogFile, options = {}): Promise<any> {
    try {
      ...
      if (!relatorioExists) {
        const findDynamicRelatorio = async (dir): Promise<any> => {
            if (!dir) return null;
            try {
                const files = await fs.readdir(dir);
      ...
      const isOnlySpecialCommand = (cmd: any): any =>
      ...
      const isFrontendFile = (file: any): any => {
      ...
      const isBackendFile = (file: any): any => {
          if (!file) return false;
          const normalized = file.replace(/\\/g, '/');
