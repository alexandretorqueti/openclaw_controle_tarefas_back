...
import taskHierarchyService from "./taskHierarchyService"; // <-- Serviço de hierarquia
...
class TaskService {
  ...
  async buildProject(projectPath): Promise<any> {
    try {
  ...
  async validateReferences(data): Promise<any> {
    const errors = [];
...
(error as any).statusCode = 400;
  ...
  async getAllTasks(filters = {}): Promise<any> {
    const where = {};
  ...
  async validateUpdateReferences(id, data): Promise<any> {
    errors = [];
...
(error as any).statusCode = 400;
  ...
  // Update task
  async updateTask(id, data): Promise<any> {
    // Validate update references
   ...
   */
  async startTaskExecution(taskId): Promise<any> {
   ...
   */
  async stopTaskExecution(taskId): Promise<any> {
  ...
  async updateTaskPosition(id, position): Promise<any> {
  ...
  async toggleTaskCompletion(id): Promise<any> {
  ...
  async getTasksByProject(projectId, filters = {}): Promise<any> {
    where = { projectId };
  ...
  async getRecurringTasksDue(): Promise<any> {
    const now = new Date();
  ...
  async markTaskAsExecuted(taskId): Promise<any> {
...
// === NOVO MÉTODO: Obter a próxima tarefa para um usuário (usado pelo Jarbas) ===
  async getNextTaskForUser(nickname): Promise<any> {
    now = new Date();
  ...
  async finalizeTask(taskId, userId, executionNotes = null): Promise<any> {
    ...
    if (existingTask.isRecurring) {
      ...
      nextExecutionAt = this.calculateNextExecution(taskDataForCalc);
      ...
      updatedTask = transactionResults[0];
    ...
    // ==========================================
    const checkAndFinalizeAncestors = async (currentTaskId, userId, finalStatusId, transactionArray): Promise<any> => {
      const ancestorsToFinalize = [];
      
      // Função recursiva interna
      const checkAncestor = async (taskId): Promise<any> => {
    ...
    transactionResults = await prisma.$transaction(transaction);
    ...
    historyRecord = transactionResults.length > 1 ? transactionResults[1] : null;
    ...
    try {
      ...
      if (ancestorsFinalized.length > 0) {
        ...
        for (const ancestorTask of ancestorsFinalized) {
          ...
          allSubtasks = await prisma.task.findMany({
            where: {
