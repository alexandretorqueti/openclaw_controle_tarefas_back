...
// Arquivo: commandUtils.js

// src/utils/commandUtils.js
// Utilitarios para analise e manipulacao de comandos shell

import * as path from 'path';
import { normalizeFsPath, extractPathTokensFromCommand, uniquePaths } from './pathUtils';
 ...
 * @returns {string}
 */
function normalizeCommandSignature(command: any): any {
 ...
 * @returns {boolean}
 */
function isInspectionCommand(command: any): any {
 ...
 * @returns {boolean}
 */
function isMutationCommand(command: any): any {
  if (!command || typeof command !== 'string') return false;
 ...
 * @returns {boolean}
 */
function looksLikeMutationCommand(command = ''): any {
