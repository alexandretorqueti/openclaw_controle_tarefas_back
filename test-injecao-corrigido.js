// Teste com INJEÇÃO VIA CONTAINER - VERSÃO CORRIGIDA
// Configura o container com mocks CORRETOS antes de usar a classe

console.log('=== TESTE COM INJEÇÃO VIA CONTAINER (CORRIGIDO) ===\n');
...
// Mock CORRETO do log - deve ser uma função async
const mockLog = {
  log: async (...args) => {
...
const contextoTeste = {
  ...
  config: {
    TASKS_DIR: "/tmp/container-corrigido",
  ...
  files: {
    promptFile: "/tmp/container-corrigido-prompt.txt",
    relatorioFile: "/tmp/container-corrigido-relatorio.txt",
    doneFile: "/tmp/container-corrigido-done.done",
    terminalLogFile: "/tmp/container-corrigido-terminal.log",
    architectPlanFile: "/tmp/container-corrigido-plano-arquiteto.txt",
    architectLogFile: "/tmp/container-corrigido-terminal-arquiteto.log",
  },
...
async function executarTesteCorrigido() {
  try {
    ...
    console.log('   log é async function?', typeof step.log.log === 'function');
    ...
    console.log('   2. Registrar mocks CORRETOS (funções async)');
    ...
    console.log('\n🎯 TESTE VIA CONTAINER (CORRIGIDO) BEM-SUCEDIDO!');
    
    // Retornar contexto para possível teste do desenvolvedor
    return resultado;
    
  } catch (error) {
    console.error('❌ ERRO NO TESTE VIA CONTAINER (CORRIGIDO):', error.message);
