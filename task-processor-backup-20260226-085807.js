  ...
  try {
    ...
    if (project.frontendPath && fs.existsSync(project.frontendPath)) {
      ...
      try {
        ...
        const branchListResult = await exec(`cd "${frontendRepo}" && git branch --list "${branchName}"`);
        const branchExists = branchListResult.stdout.trim().length > 0;
        
        if (branchExists) {
          // Branch já existe, apenas fazer checkout
          await exec(`cd "${frontendRepo}" && git checkout "${branchName}"`);
          log(`✅ Branch já existe no frontend: ${branchName}, checkout realizado`);
        } else {
    ...
    if (project.backendPath && fs.existsSync(project.backendPath)) {
      ...
      try {
        ...
        // Verificar se branch já existe localmente
        const branchListResult = await exec(`cd "${backendRepo}" && git branch --list "${branchName}"`);
        const branchExists = branchListResult.stdout.trim().length > 0;
        
        if (branchExists) {
          // Branch já existe, apenas fazer checkout
          await exec(`cd "${backendRepo}" && git checkout "${branchName}"`);
          log(`✅ Branch já existe no backend: ${branchName}, checkout realizado`);
        } else {
          // Branch não existe, criar nova
